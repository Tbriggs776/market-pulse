...
import {
  Bot, Plus, Trash2, Send, Sparkles, Brain, Zap,
  MessageSquare, Loader2, AlertTriangle, RefreshCw,
} from 'lucide-react'
...
export default function Advisor() {
  ...
  const handleSend = useCallback(async () => {
    ...
    // Optimistically add user message to visible state
    ...
    try {
      ...
      // Refetch so final persisted messages come from DB
      queryClient.invalidateQueries({ queryKey: ['advisor-conversations'] })
      if (convId) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-[260px_1fr] gap-4 h-[calc(100vh-180px)] min-h-[500px]">
      ...
      <aside className="card-elevated flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {convsQ.error && (
            <div className="p-3 text-xs text-crimson">{convsQ.error.message}</div>
          )}
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <div className={`text-xs truncate ${isActive ? 'text-gold' : 'text-ivory'}`}>
                      {c.title}
                    </div>
      ...
      <section className="card-elevated flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between p-3 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-medium text-ivory">
            ...
            {MODELS.map((m) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-[11px] transition-colors ${
                  modelKey === m.key ? 'bg-gold/15 text-gold-bright' : 'text-text-secondary hover:text-ivory'
                }`}
                title={m.desc}
              >
          ...
          {!activeId && messages.length === 0 && !streamBuffer && (
            <div className="flex flex-col items-center justify-center h-full text-center p-6">
              ...
              <p className="text-sm text-text-secondary max-w-sm mb-6">
                Portfolio-aware research and CFO-lens analysis. Your watchlist and research bench are available as context.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-md">
                {[
                  'What does my watchlist look like right now?',
                  "What's the biggest risk in my current positioning?",
                  'Walk me through the 10Y-2Y spread implications',
        ...
        <div className="border-t border-border p-3">
          <div className="flex items-end gap-2">
            <textarea
              ...
              placeholder={streaming ? 'Streaming...' : 'Ask the advisor anything about your portfolio, markets, or a research thesis.'}
              disabled={streaming}
              rows={2}
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm text-ivory whitespace-pre-wrap leading-relaxed">
          {content}
